...
Wizard_AliDDNSv3_Configure_Init() {
    ...
    if [ -f "/etc/AliDDNSv3/AliDDNSv3.cfg" ] && [ -f "/etc/AliDDNSv3/AliDDNSv3.cfg.lock" ]; then
        ...
        read -p "继续操作将会导致当前配置文件被覆盖, 是否继续配置? (y/N)" Wizard_AliDDNSv3_Configure_Init_ContinueWithExistConfig
...
建议通过复制粘贴的方式, 将这些内容输入其中:\n"
    ...
    do
        ...
        AliDDNSv3_Config_AccessKeyID=""
        read -p "请输入你的阿里云AccessKey ID: " AliDDNSv3_Config_AccessKeyID
        ...
        if [ "$?" != "0" ]; then
            ...
            read -p "是否要尝试重新输入AccessKey ID ? (Y/n)" Wizard_AliDDNSv3_Configure_Step1_Retry
...
建议通过复制粘贴的方式, 将这些内容输入其中:\n"
    ...
    do
        ...
        AliDDNSv3_Config_AccessKeySecret=""
        read -p "请输入你的阿里云AccessKey Secret: " AliDDNSv3_Config_AccessKeySecret
        ...
        if [ "$?" != "0" ]; then
            ...
            read -p "是否要尝试重新输入AccessKey Secret ? (Y/n)" Wizard_AliDDNSv3_Configure_Step2_Retry
...
后, 待解析完全生效后再使用AliDDNSv3工具.\n"
    ...
    do
        ...
        read -p "请输入你的域名: " AliDDNSv3_Config_DomainName
...
${Msg_Debug}阿里云云解析API返回如下信息:\n${Msg_Debug}${AliDDNSv3_Var_RequestResult}"
            echo -e ""
            read -p "是否要尝试重新输入域名 ? (Y/n)" Wizard_AliDDNSv3_Configure_Step3_Retry
...
后, 待解析完全生效后再使用AliDDNSv3工具.\n"
    ...
    do
        ...
        read -p "请输入你的子域名: " AliDDNSv3_Config_SubDomainName
...
${Msg_Debug}阿里云云解析API返回如下信息:\n${Msg_Debug}${AliDDNSv3_Var_RequestResult}"
            echo -e ""
            read -p "是否要自动添加此域名记录(建议前往阿里云官网手动添加) ? (Y/n) " Wizard_AliDDNSv3_Configure_Step4_AddRecordConfirm
...
${Msg_Debug}阿里云云解析API返回如下信息:\n${Msg_Debug}${AliDDNSv3_Var_RequestResult}\n"
                    read -p "是否尝试重新输入 ? (Y/N) " Wizard_AliDDNSv3_Configure_Step4_AddRecordRetry     
            ...
            echo -e ""
            read -p "确定要使用此域名作为DDNS域名么? (y/N) " Wizard_AliDDNSv3_Configure_Step4_ExistRecordConfirm
...
自定义(定制版): 按需选择, 按需购买\n"
    ...
    do
        ...
        read -p "请输入TTL值 (默认600): " AliDDNSv3_Config_TTL
        if [ "${AliDDNSv3_Config_TTL}" = "" ]; then
            echo -e "${Msg_Warning}不填写TTL值, 将会自动设定TTL值为600."
            read -p "你确定要这样做么? (y/N) " Wizard_AliDDNSv3_Configure_Step5_DefaultTTLConfirm
...
根据自己的环境, 选择一个最合适的工作模式, 会提高AliDDNS的工作效率, 降低错误率/误报率.\n"
    ...
    do
        ...
        read -p "请选择工作模式 (1-3): " AliDDNSv3_Config_WorkMode
...
如有误, 请输入N以重新开始配置流程.\n "
    ...
    read -p "所有参数是否正确 ? (y/N) " Wizard_AliDDNSv3_Configure_FinishConfirm_ConfigureConfirm
...
Menu_MainMenu(){
    ...
    echo -e " 0. 退出"
    echo -e ""
    read -p " 请输入你的选择[0-9]: " Input_MainMenu
...
Menu_AliDDNSv3_Configure(){
    ...
    echo -e ""
    read -p " 请输入你的选择[0-10]: " Input_AliDDNSv3_Configure
