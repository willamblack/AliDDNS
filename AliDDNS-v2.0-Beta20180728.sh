...
# 更新日志
# Update 20180728 BetaBuild :
# [Fix] 临时替换获取本机IP的接口为 api.ip.la （原来的Akamai在部分地区不稳定）
#       注意：如果服务器有IPV6地址，会导致出现失败
# [Mod] 调整了程序运行的机制，使得程序框架更清晰 （其实就是去掉了很多废话一样的代码）
# [New] 增加了抛出异常与结果回显的机制，同时会跟踪显示运行过程中某些环节的结果
# [New] 增加对系统时间的调整，防止因系统时间偏差导致获取不到RecordID

BuildTime="20180728 BetaBuild"
...
Switch_env_NeedInstallEnviroment="0"
...
function_Check_Root(){
	if [[ "`id -u`" = "0" ]]; then
        ...
        Switch_env_sudo_exist="0"
        echo -e "${Msg_Error}当前用户并非Root用户，且无法使用Root权限！程序无法继续运行，正在退出…… ${Font_Yellow}(失败操作：检查Root权限)${Font_Suffix}"
        echo -e "${Msg_Info}请将以下信息提交给作者，以便进行故障定位："
        id
        if [ ${Switch_env_sudo_exist} = "0" ] ; then
            echo "/usr/bin/sudo 是否存在：否"
        else
            echo "/usr/bin/sudo 是否存在：是"
        fi
        exit 1
    fi
}

function_Check_Enviroment_Stage1() {
    if [[ ! -f "/usr/bin/curl" ]] || [[ ! -f "/usr/bin/openssl" ]] || [[ ! -f "/usr/bin/nslookup" ]] || [[ ! -f "/usr/bin/ntpdate" ]]; then
        Switch_env_NeedInstallEnviroment="0"
    else
        Switch_env_NeedInstallEnviroment="1"
    fi
}

function_Check_Enviroment_Stage2() {
    if [[ ! -f "/usr/bin/curl" ]] || [[ ! -f "/usr/bin/openssl" ]] || [[ ! -f "/usr/bin/nslookup" ]] || [[ ! -f "/usr/bin/ntpdate" ]]; then
        Switch_env_NeedInstallEnviroment="0"
    else
        echo -e "环境安装失败！程序无法继续运行，正在退出…… (失败操作：验证安装环境)"
        echo -e "${Msg_Info}请将以下信息提交给作者，以便进行故障定位："
        if [ ! -f "/usr/bin/curl" ] ; then
            echo "/usr/bin/curl 是否存在：否"
        else
            echo "/usr/bin/curl 是否存在：是"
        fi
        if [ ! -f "/usr/bin/openssl" ] ; then
            echo "/usr/bin/openssl 是否存在：否"
        else
            echo "/usr/bin/openssl 是否存在：是"
        fi
        if [ ! -f "/usr/bin/nslookup" ] ; then
            echo "/usr/bin/nslookup 是否存在：否"
        else
            echo "/usr/bin/nslookup 是否存在：是"
        fi
        if [ ! -f "/usr/bin/ntpdate" ] ; then
            echo "/usr/bin/ntpdate 是否存在：否"
        else
            echo "/usr/bin/ntpdate 是否存在：是"
        fi
        exit 1
    fi
}

function_Check_SystemRelease() {
    if [ -f "/etc/redhat-release" ]; then
        ...
        echo -e "${Msg_Error}系统环境未知，无法继续执行！正在退出…… ${Font_Yellow}(失败操作：检查系统分支)${Font_Suffix}"
        echo -e "${Msg_Info}请将以下信息提交给作者，以便进行故障定位："
        cat /etc/os-release
        exit 1
    fi
...
function_Install_Enviroment(){
    if [[ ${Switch_env_NeedInstallEnviroment} = "1" ]] ; then
        ...
        if [ "${Switch_env_is_root}" = "1" ]; then
            if [ "${Switch_env_system_release}" = "centos" ]; then
                ...
                yum install curl bind-utils openssl ntpdate -y
            elif [ "${Switch_env_system_release}" = "ubuntu" ]; then
                echo -e "${Msg_Info}检测到系统分支：Ubuntu"
                echo -e "${Msg_Info}正在安装必需组件……"
                apt-get update
                apt-get install curl dnsutils openssl ntpdate -y
                ...
                echo -e "${Msg_Error}系统环境未知，无法继续执行！正在退出…… (失败操作：安装必要环境)"
                echo -e "${Msg_Info}请将以下信息提交给作者，以便进行故障定位："
                cat /etc/os-release
                exit 1
            fi
            ...
            if [ "${Switch_env_system_release}" = "centos" ]; then
                ...
                echo -e "${Msg_Error}系统环境未知，无法继续执行！正在退出…… (失败操作：安装必要环境-通过sudo)"
                echo -e "${Msg_Info}请将以下信息提交给作者，以便进行故障定位："
                cat /etc/os-release
                exit 1
            fi
        fi
        echo -e "${Msg_Info}正在校准系统时间……"
        ntpdate ntp1.aliyun.com
        if [ "$?" -eq "0" ]; then
            echo -e "${Msg_Info}校准时间成功"
        else
            echo -e "${Msg_Warning}校准时间失败，可能会导致DDNS更新失败！"
        fi
    fi
}
...
function_AliDDNS_SetConfig(){
    ...
    if [ "${Switch_AliDDNS_ExpertMode}" = "1" ]; then
        if [ "${AliDDNS_LocalIP}" = "" ]; then
            ...
            [[ -z "${AliDDNS_LocalIP}" ]] && echo -e "${Msg_Info}检测到输入空值，设置执行命令为：“curl -s api.ip.la”" && AliDDNS_LocalIP="curl -s api.ip.la"
        fi
    else
        AliDDNS_LocalIP="curl -s api.ip.la"
    fi
    # AliDDNS_DomainServerIP
    if [ "${Switch_AliDDNS_ExpertMode}" = "1" ]; then
...
function_document_AliDDNS_LocalIP(){
    echo -e "${Msg_Info}${Font_Green}AliDDNS_LocalIP 说明${Font_Suffix}       
       这个参数决定如何获取到本机的IP地址。
       出于稳定性考虑，默认使用api.ip.la作为获取IP的方式，
...
function_AliDDNS_GetLocalIP(){
    ...
    if [ "${AliDDNS_LocalIP}" = "" ]; then
        ...
        echo -e "${Msg_Info}请将以下信息提交给作者，以便进行故障定位："
        echo "AliDDNS_LocalIP参数值：${AliDDNS_LocalIP}"
        echo "AliDDNS_LocalIP在配置文件中的参数值：`sed '/^AliDDNS_LocalIP=/!d;s/.*=//' /etc/OneKeyAliDDNS/config.cfg | sed 's/\"//g'`"
        exit 1
    fi
    ...
    if [ "${AliDDNS_LocalIP}" = "" ]; then
        ...
        echo -e "${Msg_Fail}程序运行出现致命错误，正在退出……"
        echo -e "${Msg_Info}请将以下信息提交给作者，以便进行故障定位："
        echo "获取本机IP返回结果：${AliDDNS_LocalIP}"
        exit 1
    else
...
function_AliDDNS_UpdateRecord(){
    ...
    if [ "${AliDDNS_RecordID}" = "" ]; then
        ...
        if [ "${AliDDNS_RecordID}" = "" ]; then
            echo -e "${Msg_Warning}未能获取到RecordID，可能没有检测到有效的解析记录 (RecordID：[空])"
        else
            ...
            update_record $AliDDNS_RecordID > /tmp/AliDDNS_Debug_RequestACK.txt
            echo -e "${Msg_Info}更新记录响应结果 -> `cat /tmp/AliDDNS_Debug_RequestACK.txt | head -n1`"
            echo -e "${Msg_Info}RecordID <$AliDDNS_RecordID> 更新记录成功"
        fi
    fi
    if [ "${AliDDNS_RecordID}" = "" ]; then
        ...
        echo "AliDDNS_TTL：$AliDDNS_TTL"
        echo "AliDDNS_AK：$AliDDNS_AK"
        echo "AliDDNS_SK：$AliDDNS_SK"
        echo "AliDDNS_LocalIP：$AliDDNS_LocalIP"
        echo "AliDDNS_DomainServerIP：$AliDDNS_DomainServerIP"
        echo ""
        echo "配置文件中的参数："
        cat /etc/OneKeyAliDDNS/config.cfg
        echo ""
        echo "args：`cat /tmp/AliDDNS_Debug_args.txt | head -n1`"
        echo ""
        echo "hash：`cat /tmp/AliDDNS_Debug_hash.txt | head -n1`"
        echo ""
        echo "RequestURL：http://alidns.aliyuncs.com/?`cat /tmp/AliDDNS_Debug_args.txt | head -n1`&Signature=$(enc "`cat /tmp/AliDDNS_Debug_hash.txt | head -n1`")"
        echo ""
        echo -e "RequestACK：`cat /tmp/AliDDNS_Debug_RequestACK.txt` \n"
        echo "RecordID：$AliDDNS_RecordID"
        echo "timestamp：$timestamp"
        rm -f /tmp/AliDDNS_Debug_*.txt
        exit 1
    else
        # 输出成功结果
...
function_ServerChan_SuccessMsgPush(){
    ...
    if [ "${Switch_ServerChan_ConfigExist}" = "1" ]; then
        if [ ${Switch_ServerChan_Enable} = "1" ]; then
            ...
            curl -s "http://sc.ftqq.com/$ServerChan_SCKEY.send?text=${ServerChan_Text}" -d "&desp=${ServerChan_Content}" > /tmp/AliDDNS_Debug_ServerChanPushMsg.txt
            echo -e "${Msg_Info}ServerChan响应结果：`cat /tmp/AliDDNS_Debug_ServerChanPushMsg.txt | head -n1`"
            if [ "$?" -eq "0" ]; then
                echo -e "${Msg_Success}ServerChan 推送成功，服务器IP变动消息已经送达微信，请注意查收"
            else
